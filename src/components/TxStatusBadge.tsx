...
interface Props {
  step: 'idle' | 'signing' | 'sending' | 'confirming' | 'success' | 'error'
...
export default function TxStatusBadge({ step, errorMsg, txHash }: Props) {
  ...
  const configs = {
    signing:    { bg: '#F5F3FF', border: '#C4B5FD', shadow: '#8B5CF6', text: '#5B21B6', label: 'Securing EIP-712 signature...' },
    ...
    success:    { bg: '#ECFDF5', border: '#6EE7B7', shadow: '#10B981', text: '#065F46', label: 'Confirmed onchain' },
  ...
  return (
    ...
    >
      <span className="font-body font-bold text-sm" style={{ color: c.text }}>{c.label}</span>
      {step === 'success' && txHash && (
        <a
