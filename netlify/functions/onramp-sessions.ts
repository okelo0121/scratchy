...
// CORS headers — tighten to your domain in production
const CORS = {
...
export default async function handler(req: Request, _ctx: Context): Promise<Response> {
  ...
  // ── Attempt to use @circle-fin/onramp-kit/server ────────────────────────────
  // The package is in private beta; wrap in try/catch so the function degrades
  // gracefully until the Cloudsmith key is configured and `bun add` is run.
  try {
    // Dynamic import so a missing module is a runtime error, not a build error
    ...
    // Missing module = private beta package not yet installed
    if (msg.includes('Cannot find module') || msg.includes('ERR_MODULE_NOT_FOUND') || msg.includes('MODULE_NOT_FOUND')) {
      return new Response(
        JSON.stringify({
          ...
          detail: '@circle-fin/onramp-kit is not installed. Ask your Circle contact for the Cloudsmith key, then run: bun add @circle-fin/onramp-kit',
        }),
        { status: 503, headers: { ...CORS, 'Content-Type': 'application/json' } },
      )
    }
